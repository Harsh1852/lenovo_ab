// ============================================================
//  Lenovo Clone – Variant A (Sidebar Filter)
...
function initSidebarFilters() {
  ...
  ['price-min', 'price-max'].forEach(id => {
    ...
    if (el) el.addEventListener('input', () => {
      filters.priceMin = parseInt(document.getElementById('price-min').value) || 0;
      filters.priceMax = parseInt(document.getElementById('price-max').value) || 2000;
...
function syncFiltersFromCheckboxes() {
  const get = group => [...document.querySelectorAll(`.filter-checkbox[data-group="${group}"]:checked`)].map(c => c.value);
  filters.series        = get('series');
  filters.processorBrand= get('processorBrand');
  filters.ram           = get('ram').map(Number);
  filters.storage       = get('storage');
  filters.screen        = get('screen').map(Number);
...
function renderActiveFilterTags() {
  ...
  if (!bar) return;
  const tags = [];
  const push = (group, val, label) => tags.push({group, val, label});
  ...
  filters.screen.forEach(v        => push('screen',          v, v + '" Screen'));
  ...
  bar.innerHTML =
    ...
    tags.map(t =>
      `<button class="filter-tag" onclick="removeFilter('${t.group}','${t.val}')">${t.label} ×</button>`
    ).join('') +
    `<button class="tag-clear-all" onclick="clearAllFilters()">Clear All</button>`;
}

function removeFilter(group, rawVal) {
  const val = ['ram','screen'].includes(group) ? Number(rawVal) : rawVal;
  const cb = document.querySelector(`.filter-checkbox[data-group="${group}"][value="${rawVal}"]`);
...
function clearAllFilters() {
  ...
  filters = { series:[], processorBrand:[], ram:[], storage:[], screen:[], type:[], priceMin:0, priceMax:2000 };
  sortBy = 'relevance';
  const sortEl = document.getElementById('sort-select');
...
function updateClearBtn() {
  ...
  if (!btn) return;
  const hasFilter = Object.values(filters).some(v => Array.isArray(v) ? v.length : false)
...
function renderCompareTray() {
  ...
  if (!tray) return;
  if (ids.length === 0) { tray.classList.remove('visible'); return; }
